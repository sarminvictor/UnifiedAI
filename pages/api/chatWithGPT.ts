...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const messages = [
      ...
      ...previousMessages.map((msg: ChatHistoryType) => 
      ...
      ...previousMessages.map((msg: { user_input: string | null; api_response: string | null }) => 
        msg.user_input 
    ...
    // Initialize model and chain with memory - remove verbose
    const llm = getAIModel(modelName);
    const chain = new LLMChain({
      llm,
      prompt,
      memory,
      verbose: false  // Set to false to remove logs
    });
    ...
    if (process.env.NODE_ENV === 'development') {
      console.error('ChatWithGPT Error:', { message: error.message });
    }
